...
addParamValue(p, 'alpha',  0.5, @isscalar);
...
if ~isempty(p.Results.fixed) && ~isempty(p.Results.r0) && ...
    ...
    penalty_fix = 1e3 * p.Results.alpha * sum( (p.Results.r0(p.Results.fixed) - p.Results.r(p.Results.fixed)).^2);
else
    penalty_fix = 0;
end
...
rss = diff(r, 2);
maxX = size(edge_img,1) ;
maxY = size(edge_img,2) ;
